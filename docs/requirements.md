...
| NFR-009 | Read-Only Public Access   | The content backend token shipped with the website must grant only read access to published content and submission of the three forms, and no write access to any other content. | Security        | High     | Implemented |
...
| C-003 | Content Backend              | All content must be read from the Cockpit content management system at `cockpit.cevi-buro-aarau.ch`; the website has no own database. | Technical   | High     | Implemented |
| C-004 | Form Delivery Channel        | The three forms must be submitted to the Cockpit form endpoints `join`, `contact` and `shoporder`.                                 | Technical   | High     | Implemented |
| C-005 | Album Download Service       | Whole albums must be downloaded through the separate download service at `hook.cevi-buro-aarau.ch`.                                | Technical   | Medium   | Implemented |
...
| C-008 | Public Backend Token         | The backend token is contained in the delivered website and is therefore public; no secret may be shipped with the frontend.       | Technical   | High     | Implemented |
